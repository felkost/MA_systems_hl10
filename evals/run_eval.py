...
class CaseResult:
    ...
    `run_index` (stage 10b, decision D1) is 1 on a plain single-pass call
    and counts up to `repeat` under `--repeat N` -- the field that lets
    `evals/statistics.py` attribute n >= 3 runs to one configuration rather
    than a mixture (spec Sec13.9). `metrics` is `evals.metrics.CaseMetrics`
    serialised via `dataclasses.asdict`, populated whenever a pack could be
    assembled (independent of whether the judge ran -- D1's whole point).
    """
...
async def _drive_case(
    ...
    *,
    ...
    ----------
    thread_id : str
        A fresh id per case (see module docstring) -- never reused, never
        derived from the case id, so two runs of the same case never
        collide on the checkpointer. Generated by the caller, not here, so
        a case that raises before this function returns can still be
        reported under the thread_id it was attempted on.
    question : str
...
def _persist_pack(
    run_dir: Path, run_index: int, case_id: str, pack: evidence.EvidencePack
) -> None:
    """Write `pack` to `run_dir/packs/<run_index>__<case_id>.json` (D1) --
    persisted whether or not the judge runs, so `score.py` can judge (and
    re-judge, under a revised rubric) offline, off a fixed corpus, without
    re-running the live stack."""
    packs_dir = run_dir / "packs"
    packs_dir.mkdir(parents=True, exist_ok=True)
    path = packs_dir / f"{run_index:02d}__{case_id}.json"
    path.write_text(json.dumps(asdict(pack)), encoding="utf-8")


def _dataset_digest(cases: Sequence[GoldenCase]) -> str:
    """A short, stable fingerprint of exactly which cases (and their
    content) this run covers -- part of `run_metadata.json` (spec Sec5.1),
    so two run-sets can be shown to have swept the same dataset rather than
    two silently different edits of it."""
...
def _git_revision() -> str | None:
    """The checked-out commit, best-effort -- `None` (never a raised
    exception) when `git` is unavailable or this is not a checkout, since a
    run-set's provenance is a nice-to-have, not a reason to fail a paid
    run."""
    try:
...
async def _run_one_case(
    ...
    *,
    ...
    run_dir: Path,
    span_dump_dir: Path,
    fixture_server: "FixtureServer | None",
    run_judge: bool,
    judge_model: BaseChatModel | None,
    dataset_linker: "DatasetLinker | None",
    run_name: str,
) -> CaseResult:
    """One (run_index, case) attempt: drive, persist the pack, optionally
    judge, optionally link -- wrapped so a raise anywhere in this sequence
    becomes an error `CaseResult` instead of aborting the whole sweep
    (spec Sec5.1)."""
    ...
    try:
        ...
        result = await _drive_case(
            ...
            question,
            thread_id=thread_id,
            span_dump_dir=span_dump_dir,
        )
        result.run_index = run_index

        if result.trace_id is not None:
            pack = evidence.assemble(
                span_dump_dir, result.trace_id, final_answer=result.final_answer
            )
            _persist_pack(run_dir, run_index, case.id, pack)
            result.metrics = asdict(metrics_of(pack))
            if run_judge:
        ...
        return result
    except Exception as error:  # noqa: BLE001 -- one case's crash must not
        # abort the sweep; the exception is recorded as data, logged for a
        # human to read afterward, never silently swallowed.
        logging.getLogger("main").exception(
            "case %r (run %d) failed: %s", case.id, run_index, error
        )
        return CaseResult(
            case_id=case.id,
            thread_id=thread_id,
...
async def run_dataset(
    ...
    *,
    ...
    `repeat` times over, writing one JSON line per (run, case) pair to
    `run_dir / "results.jsonl"` as soon as it is known.
    ...
    run_dir : Path
        Created if missing; holds `results.jsonl`, `packs/` and (via
        `settings`) the span dump and output directory.
    run_judge : bool, default False
        Off by default (stage 10b, decision D1): running and judging are
        separate passes now -- `evals/score.py` judges persisted packs
        offline, against a rubric version that may not exist yet when the
        (expensive, real) run happens. Set `True` only for a smoke-style
        run that wants a verdict inline.
    repeat : int, default 1
        Loop the whole `cases` sequence this many times against the *same*
        boot of the stack (stage 10b, decision D1/D3) -- six processes at
        stage-9's measured minimum free RAM, and a ~31 s two-phase boot, do
        not survive a boot per repeat any more than they survive one per
        case. Each `CaseResult.run_index` (1-based) records which pass it
        came from, and every pack is namespaced `<run_index>__<case_id>` so
        repeats never collide on disk.
    ...
    -------
    list of CaseResult
        `repeat * len(cases)` entries. A case that raises is recorded with
        `error` set and every other field at its default rather than
        aborting the sweep (spec Sec5.1: "a crash in run 2 case 7 must not
        cost runs 1 and 3").
    """
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--judge",
        action="store_true",
        help="Run the judge inline for every case (stage 10b, decision D1: "
        "off by default -- running and judging are separate passes now, "
        "see `python -m evals.score`)",
    )
    parser.add_argument(
        "--repeat",
        type=int,
        default=1,
        help="Loop the whole dataset this many times against one boot of "
        "the stack (stage 10b, decision D1); default 1",
    )
    parser.add_argument(
        "--no-dataset-link",
        action="store_true",
